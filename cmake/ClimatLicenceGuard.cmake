...
function(_climat_banned_reason module out)
    set(${out} "GPLv3-or-commercial from The Qt Company; linking it relicenses \
the whole application and breaks libclimat's MPL-2.0 promise (docs/08-risks.md, R2)"
        PARENT_SCOPE)
endfunction()

# Call before find_package(Qt6 ...) with the same component list.
function(climat_guard_qt_components)
    foreach(component IN LISTS ARGN)
        if(component IN_LIST CLIMAT_BANNED_QT_MODULES)
            _climat_banned_reason(${component} reason)
            message(FATAL_ERROR
...
function(climat_guard_target target)
    get_target_property(libs ${target} LINK_LIBRARIES)
    if(NOT libs)
        return()
    endif()
    foreach(lib IN LISTS libs)
        foreach(banned IN LISTS CLIMAT_BANNED_QT_MODULES)
            if(lib STREQUAL "Qt6::${banned}" OR lib STREQUAL "Qt::${banned}")
                _climat_banned_reason(${banned} reason)
                message(FATAL_ERROR
                    "Target '${target}' links ${lib}, which is not permitted in Climat.\n"
                    "  ${reason}")
            endif()
        endforeach()
    endforeach()
