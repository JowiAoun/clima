...
#
# Refuses to configure a build in which libclimat can see a GUI.
#
# docs/04-architecture.md §4.1, design principle 3: "The engine has no GUI
# dependency. `libclimat` links Qt Core/Network only, so it can be reused by a
...
# why `libclimat` is MPL-2.0 and GUI-free."
#
...
set(CLIMAT_GUI_QT_TARGETS
    ...
    Qml            # not a windowing dependency, but the engine must not need a
                   ...
                   # their own, and a libclimat that registers QML types has
                   # decided for them.
    OpenGL
    OpenGLWidgets
    PrintSupport
    Svg
    SvgWidgets
    CACHE INTERNAL "Qt targets libclimat may never reach, directly or transitively"
)
...
function(_climat_walk_link_closure target out_visited out_offenders out_paths)
    ...
    while(queue)
        ...
        foreach(banned IN LISTS CLIMAT_GUI_QT_TARGETS)
        ...
        foreach(property IN LISTS link_properties)
            ...
            foreach(dependency IN LISTS dependencies)
                _climat_normalise_link_entry("${dependency}" normalised)
...
function(climat_forbid_gui target)
    _climat_walk_link_closure(${target} visited offenders paths)
