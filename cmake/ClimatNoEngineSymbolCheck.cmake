...
#
#   cmake -DCLIMAT_BINARY=<path> -DCLIMAT_NM=<nm> -P ClimatNoEngineSymbolCheck.cmake
#
...
if(NOT CLIMAT_BINARY)
    message(FATAL_ERROR "ClimatNoEngineSymbolCheck: pass -DCLIMAT_BINARY=<path>")
endif()

if(NOT EXISTS "${CLIMAT_BINARY}")
    message(FATAL_ERROR "ClimatNoEngineSymbolCheck: no such file: ${CLIMAT_BINARY}")
endif()

if(NOT CLIMAT_NM OR NOT EXISTS "${CLIMAT_NM}")
    message(FATAL_ERROR "ClimatNoEngineSymbolCheck: CLIMAT_NM is not a usable program: ${CLIMAT_NM}")
endif()

execute_process(
    COMMAND "${CLIMAT_NM}" --defined-only "${CLIMAT_BINARY}"
...
if(NOT symbol_status EQUAL 0)
    message(FATAL_ERROR "ClimatNoEngineSymbolCheck: nm failed: ${symbol_errors}")
endif()

# ---- the positive control ----------------------------------------------------
#
# `climat::scales::uvBand` is compiled into the widget host by
# widgets/CMakeLists.txt and called by the UV dial. If it is not in the table
# then the table is not telling us anything.
if(NOT symbols MATCHES "climat6scales")
    message(FATAL_ERROR
        "\n"
        "  ClimatNoEngineSymbolCheck could not read a symbol table from\n"
        "  ${CLIMAT_BINARY}.\n"
        "\n"
        "  It expected to find climat::scales, which the UV and air-quality tiles\n"
...
# `5climat10HttpClient` for climat::HttpClient, and so on. Substrings rather than
# anchored patterns, because the surrounding mangling depends on the member.
set(forbidden
    "climat10HttpClient"
    "climat10CacheStore"
    "climat16ProviderRegistry"
    "climat24OpenMeteoForecastProvider"
    "climat20MetNoForecastProvider"
    "climat26OpenMeteoAirQualityProvider"
    "climat17EcccAlertProvider"
    "climat16NwsAlertProvider"
    "climat13GeoNamesIndex"

    # And the two that catch linking libclimat *without* calling into it.
    #
    ...
    "qInitResources_climat_geonames"
    "qInitResources_climat_fixtures"
)
...
if(offenders)
    ...
    message(FATAL_ERROR
        "\n"
        "  ${CLIMAT_BINARY} carries the engine's fetching half, and it must not.\n"
        "\n"
        "  Found (mangled-name fragments):\n    ${printable}\n"
        "\n"
        "  A widget process draws what climat-daemon fetched. Six tiles must not be\n"
...
message(STATUS "ClimatNoEngineSymbolCheck: ${CLIMAT_BINARY} carries no provider, cache or HTTP symbol")
