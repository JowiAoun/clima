...
void AppTranslator::install(QCoreApplication *app)
{
    ...
    // "climat_" is the prefix scripts/i18n.sh names them with. QTranslator
    // walks the locale's own fallback chain for us - pt_BR then pt, fr_CA then
    // fr - so a build carrying `climat_fr.qm` serves a Quebec desktop without
    // anybody listing fr_CA anywhere.
    if (!translator->load(QLocale(), QStringLiteral("climat"), QStringLiteral("_"),
                          QStringLiteral(":/i18n"))) {
