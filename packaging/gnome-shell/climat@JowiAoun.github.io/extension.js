...
//
// Climat's GNOME Shell extension. It draws no weather.
//
// ============================================================================
...
// docs/widgets.md in the Climat repository has the numbers.
//
...
// the shell's own theme. They cannot use Climat's typeface, its colour tokens or
...
const DAEMON_NAME = 'io.github.JowiAoun.Climat.Daemon';
const DAEMON_PATH = '/io/github/JowiAoun/Climat/Daemon';
const DAEMON_IFACE = 'io.github.JowiAoun.Climat.Daemon1';

const APP_ID = 'io.github.JowiAoun.Climat';
...
class DaemonStarter {
    ...
    _launch() {
        ...
        const dev = GLib.getenv('CLIMAT_DAEMON');
        const host = dev || GLib.find_program_in_path('climat-daemon');

        let argv;
        if (host) {
            argv = [host];
        } else if (GLib.find_program_in_path('flatpak')) {
            argv = ['flatpak', 'run', '--command=climat-daemon', APP_ID];
        } else {
            return;
        }
...
class WidgetHost {
    ...
    _command() {
        ...
        const dev = GLib.getenv('CLIMAT_WIDGET');
        if (dev)
            return [dev, ...args];

        const host = GLib.find_program_in_path('climat-widget');
        if (host)
            return [host, ...args];

        const flatpak = GLib.find_program_in_path('flatpak');
        if (flatpak && this._flatpakInstalled())
            return [flatpak, 'run', `--command=climat-widget`, APP_ID, ...args];
...
const ClimatIndicator = GObject.registerClass(
class ClimatIndicator extends PanelMenu.Button {
    _init(settings) {
        super._init(0.5, 'Climat');
        ...
        this._place = new PopupMenu.PopupMenuItem(_('Climat'), {reactive: false});
        ...
        const open = new PopupMenu.PopupMenuItem(_('Open Climat'));
        ...
        this._proxy.SchemaVersionRemote(([version], error) => {
            ...
            if (version !== UNDERSTOOD_SCHEMA) {
                this._status.label.text =
                    _('This extension and the Climat app are different versions.');
                this._label.text = '⚠';
                return;
            }
            this._subscribe();
    ...
    _onVanished() {
        ...
        this._proxy = null;
        this._signalId = 0;
        this._status.label.text = _('Climat is not running.');
    }
    ...
    _openApp() {
        ...
        if (app === null) {
            log_('no desktop entry for the Climat app.');
            return;
        }
...
export default class ClimatExtension extends Extension {
    enable() {
        ...
        if (this._settings.get_boolean('show-indicator')) {
            this._indicator = new ClimatIndicator(this._settings);
