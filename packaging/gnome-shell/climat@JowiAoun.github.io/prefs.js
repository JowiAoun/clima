...
//
// `climat-widget --list`, run once when this window opens. Not a hard-coded
...
function catalogue() {
    ...
    const host = dev || GLib.find_program_in_path('climat-widget');

    let argv;
    if (host) {
        argv = [host, '--list'];
    } else if (GLib.find_program_in_path('flatpak')) {
        argv = ['flatpak', 'run', '--command=climat-widget', APP_ID, '--list'];
    } else {
        return [];
    }
...
export default class ClimatPreferences extends ExtensionPreferences {
    fillPreferencesWindow(window) {
        ...
        if (known.length === 0) {
            tiles.add(new Adw.ActionRow({
                title: _('Climat is not installed'),
        ...
        const panel = new Adw.PreferencesGroup({
            ...
            description: _('The indicator is drawn by GNOME Shell, so it uses the '
                           + 'shell’s own theme and typeface rather than Climat’s.'),
        });
        page.add(panel);
