...
#include "libclimat/net/validatorstore.h"
...
Q_LOGGING_CATEGORY(lcHttp, "climat.net.http")
...
{
    // Built once, from CMake's variables. See libclimat/climatidentity.h.in for
    // why every piece of this is generated rather than written down.
    static const QByteArray agent = QByteArrayLiteral("Climat/") + CLIMAT_ENGINE_VERSION
        + QByteArrayLiteral(" (+") + CLIMAT_PROJECT_URL
        + QByteArrayLiteral("; ") + CLIMAT_CONTACT + QByteArrayLiteral(")");
    return agent;
}
