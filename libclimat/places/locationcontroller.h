...
// (libclimat/cache/migrations.cpp explains why that is not app/settings.h): the
// answer has to travel with the cache it describes, so that a Plasma applet and
// a CLI reusing libclimat open on the same place the app did. A QSettings entry
...
//
// app/qml/Climat/LocationBar.qml already emits `changeRequested()` and
