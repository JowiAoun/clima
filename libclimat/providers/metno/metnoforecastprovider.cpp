...
// libclimat/domain/weathercode.h beside the one the chart's header band folds a
...
Attribution MetNoForecastProvider::attribution() const
{
    ...
    credit.note        = QStringLiteral(
        ...
        "conditional requests; Climat sends both. Used as the automatic fallback when the "
...
QFuture<Result<Forecast>> MetNoForecastProvider::fetchForecast(const ForecastRequest &request)
{
    ...
    // (libclimat/cache/payloadcache.h). It matters more here, not less: the
