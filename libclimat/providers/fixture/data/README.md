...
`tests/fixtures/openmeteo/record.sh`, and compiled into `libclimat` at `:/climat/fixtures/<name>/`.
...
is to make `climat --fixture toronto` come up with a screen that is the same screen next month.
...
one timestamp, made through `libclimat/core/clock.h`. Freeze it and all of them freeze together, with
...
`libclimat/providers/openmeteo/openmeteovariables.cpp` - a fixture that does not carry a series the
